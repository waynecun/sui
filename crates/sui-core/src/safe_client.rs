...
impl<C> SafeClient<C> {
    ...
    fn check_object_response(
        ...
        let certificate = match parent_certificate {
            Some(certificate) => {
                Some(certificate.verify(&self.get_committee(&certificate.auth_sign_info.epoch)?)?)
            }
            None => None,
        };
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    pub async fn handle_transaction(
        ...
        let transaction_info =
            match self.check_transaction_response(&digest, None, transaction_info) {
                Err(err) => {
    ...
    pub async fn handle_certificate(
        ...
        let transaction_info = match self.verify_certificate_response(&digest, &transaction_info) {
            Err(err) => {
                self.report_client_error(&err);
                return Err(err);
            }
    ...
    pub async fn handle_object_info_request(
        ...
        let response = self
            ...
            .tap_err(|err| self.report_client_error(err))?;
